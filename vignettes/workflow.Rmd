...
```{r message=FALSE, include=FALSE, warning=FALSE, results='hold' }
balanced_data <- read.csv(system.file(package = "SentiAnalyzer", "extdata/balanced_data.csv"))
str(balanced_data)
dim(balanced_data)
```
...
SentiAnalyzer::CleanText(source_dataset=balanced_data, dtm_method=1, reductionrate=0.99)
```
example:
```{r message=FALSE, include=FALSE, warning=FALSE, results='hold'}
dim(clean_dataset)
