...
**input**: document-term matrix (e.g., result from `CleanText()` function which is `cleaned_dataset`) or list of trained models. Output of `SentiAnalyzer::BuildTraining()`  (e.g., `trained_models` saved in package data)

**output**: list of various parameter values of 5 machine learning algorithms (GBM, KNN, NB, RF,SVM_Poly)


Here we used `BuilPrediction` for an already trained object that we load earlier (e.g.,`trained_models`). As mentioned earlier the input can also be a cleaned document-term matrix (e.g.,output of `CleanText()` function)
