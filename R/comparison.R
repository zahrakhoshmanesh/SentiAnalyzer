...
comparison <- function(x) {
  ...
  f1score <- function(x) {
    2 * ((x$table[1]) / (x$table[1] + x$table[1, 2])) * ((x$table[1]) / (x$table[1] + x$table[2, 1])) / (((x$table[1]) / (x$table[1] + x$table[1, 2])) + (x$table[1]) / (x$table[1] + x$table[2, 1]))
  }
  
  
  
  df <- df %>% dplyr::mutate(
    accuracy = purrr::map(.x = conf, .f = accuracy_f),
  ...
  df <- df[-1] %>% 
    purrr::map(.f = funm) %>% 
    as.data.frame() %>% 
    dplyr::mutate(method = c("gbm", "KNN", "NB", "RandomForest", "SVM"))
